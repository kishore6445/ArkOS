...
export function DepartmentPage({ config, departmentKey }: DepartmentPageProps) {
  ...
  const handleIncrementPowerMove = (id: string, nextProgress: number) => {
    setPowerMoves((prev) =>
      prev.map((pm) => {
        if (pm.id !== id) return pm
        const targetPerCycle = Math.max(pm.targetPerCycle || 0, 1)
        const clampedProgress = Math.min(nextProgress, targetPerCycle)
        return {
          ...pm,
          progress: clampedProgress,
          weeklyActual: clampedProgress,
          activityCompleted: clampedProgress >= targetPerCycle,
        }
      }),
    )
  }
      ...
      <div className='max-w-7xl mx-auto px-4 py-8 bg-white'>
        ...
        <div>
          ...
          {filteredPowerMoves.length > 0 ? (
            <PowerMovesTable powerMoves={filteredPowerMoves} onIncrementPowerMove={handleIncrementPowerMove} />
          ) : (
