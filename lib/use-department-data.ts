      ...
      try {
        ...
        const filteredTargets = Array.isArray(vtResult.targets)
          ? vtResult.targets.filter((target: DepartmentVictoryTarget) => {
              const matchesBrand = !target.brandId || target.brandId === currentBrand
              const matchesDepartment = target.department === department
        ...
        const filteredPowerMoves = Array.isArray(pmResult.powerMoves)
          ? pmResult.powerMoves.filter((pm: DepartmentPowerMove) => {
              const matchesBrand = !pm.brandId || pm.brandId === currentBrand
              const matchesDepartment = pm.department === department
              return matchesBrand && matchesDepartment
            })
          : []
